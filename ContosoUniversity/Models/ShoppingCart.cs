...
namespace ContosoUniversity.Models
{
    public class ShoppingCart
    {
        ...
        {
            var cartItem = db.CartItems.FirstOrDefault(c => c.CartID == ShoppingCartId && c.Book.ID == book.ID);
            if (cartItem == null)
            {
        ...
        public void RemoveFromCartBtn(int id, ApplicationDbContext db)
        {
            var cartItem = db.CartItems.SingleOrDefault(cart => cart.CartID == ShoppingCartId && cart.Book.ID == id);
        ...
        {
            List<CartItem> cartItems = db.CartItems.Include(i => i.Book).Where(cartItem => cartItem.CartID == ShoppingCartId).ToList();
